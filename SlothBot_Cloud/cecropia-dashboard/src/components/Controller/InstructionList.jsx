  ...
  render() {
    ...
    if (this.state.agentDataReady) {
      ...
      for (let instruction of data.instructions) {
        instructions.push(
          <ListGroupItem uniqueKey={uniqueKey++}>{instruction}</ListGroupItem>
        );
      }
      if (instructions.length === 0) {
        instructions.push(
          <ListGroupItem key="empty">Instruction Queue is empty</ListGroupItem>
        );
      }
    } else {
      instructions = <ListGroupItem>Loading...</ListGroupItem>;
    }
    return <ListGroup className="black">{instructions}</ListGroup>;
  }
}
